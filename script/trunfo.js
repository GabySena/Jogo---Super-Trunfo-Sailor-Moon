...
function sortearCartas() {
    ...
    cartasJogador.forEach(carta => {
        divCartasJogador.innerHTML += `
            <div class="carta" onclick="escolherCartaJogador(${JSON.stringify(carta).replace(/"/g, '&quot;')})">
                <img src="${carta.imagem}" alt="${carta.nome}" width="170px">
                <p>${carta.nome}</p>
            </div>
        `;
    });
    ...
    cartasComputador.forEach(carta => {
        divCartasComputador.innerHTML += `
            <div class="carta">
                <img src="${carta.imagem}" alt="${carta.nome}" width="150px" style="visibility: hidden;">
...
function iniciarCombate(cartaJogador, cartaComputador, atributoEscolhido) {
    ...
    shaw1.innerHTML = `
        <img src="${cartaJogador.imagem}" alt="${cartaJogador.nome}" class="carta-joga">
    ...
    shaw2.innerHTML = `
        <img src="${cartaComputador.imagem}" alt="${cartaComputador.nome}" class="carta-pc">
...
function escolherCartaJogador(carta) {
    ...
    const shaw1 = document.querySelector('.shaw1');
    shaw1.innerHTML = `
        <img src="${carta.imagem}" alt="${carta.nome}" width="150px">
        <p>${carta.nome}</p>
    `;
